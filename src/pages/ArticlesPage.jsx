  ...
  return (
    <PageShell>
      ...
      <section className="mx-auto w-full max-w-[1200px] px-5 pb-10 pt-10 md:px-8 md:pt-12">
        ...
        <h1 className="text-[34px] font-bold leading-[1.1] tracking-tight text-[#172126] md:text-[48px]">Office pod articles and planning guides</h1>
        ...
        {ARTICLES.map((article) => (
          <article key={article.slug} className="overflow-hidden rounded-[10px] border border-[#ddd8cf] bg-white">
            ...
            <div className="p-6">
              ...
              <h2 className="mt-3 text-[25px] font-semibold tracking-tight text-[#172126]">{article.title}</h2>
              <p className="mt-3 text-[16px] leading-[1.6] text-[#59635f]">{article.excerpt}</p>
              <Link to={`/articles/${article.slug}`} className="mt-4 inline-flex text-[15px] font-semibold text-[#007653] underline-offset-4 hover:underline">
                Read article
              </Link>
            </div>
          </article>
